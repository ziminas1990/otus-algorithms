...
{
public:
  uint8_t getCode(uint8_t nSymbol) const {
    assert(nSymbol >= at(0));
...
using SwitchesTable = std::vector<Switches>;
...
{
  SwitchesTable table;
  table.reserve(nLength);

  for (size_t i = 1; i <= nLength; ++i)
    table.push_back(precalculateSwitches(sPattern, i, alphabet));
                                           ...
                                           Alphabet const& alphabet)
{
  ...
  for(size_t i = 0; i < sText.size(); ++i) {
    ...
    assert(nState < table.size());
    nState = table[nState][alphabet.getCode(sText[i])];
  }
